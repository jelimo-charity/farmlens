...
export function ClimateImpactStep({ form, climateEvents }: ClimateImpactStepProps) {
  const { register, watch, formState: { errors } } = form;
  const lossValue = watch("estimatedLossPercentage") ?? 0;

  return (
    <div className="space-y-4">
      <h2 className="text-base font-semibold text-gray-900">Climate Impact</h2>

      <Field label="Climate Event" htmlFor="climateEvent" error={errors.climateEvent?.message}>
        <select id="climateEvent" className={inputClass} {...register("climateEvent")}>
          <option value="">Select climate event</option>
          {climateEvents.map((c) => (
...
<p className="text-xs text-gray-500">
  ...
  climate event.
      ...
      <Field label="Photo URL" htmlFor="imageUrl" optional error={errors.imageUrl?.message}>
        <input
          id="imageUrl"
          className={inputClass}
